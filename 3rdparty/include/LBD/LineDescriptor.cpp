...
LineDescriptor::~LineDescriptor(){
//	cout<<"numOfOctave_="<<numOfOctave_<<endl;
	for(unsigned int i=0; i<numOfOctave_; i++){
//		cout<<"*************************************"<<endl;
//		cout<<i<<endl;
		if(edLineVec_[i] !=NULL){
//			cout<<"NULL="<<NULL<<endl;
//			cout<<i<<" "<<edLineVec_[i]<<endl;
//			cout<<edLineVec_[i]->imageWidth<<endl;
			delete edLineVec_[i];
//			cout<<"delete "<<edLineVec_[i]<<endl;
		}
//		cout<<"*************************************"<<endl;
//		cout<<endl;
	}
...
//		cv::waitKey();
		...
		//detect line for each octave img;
		if(!edLineVec_[octaveCount]->EDline(blur,true)){
			return -1;
		}
		numOfFinalLine += edLineVec_[octaveCount]->lines_.numOfLines;
//		cout<<"edLineVec_["<<octaveCount<<"]->lines_.numOfLines="<<edLineVec_[octaveCount]->lines_.numOfLines<<endl;
//		cout<<"numOfFinalLine="<<numOfFinalLine<<endl;

		////////////////////////////////////
		//down sample the current octave img to get the next octave img
		img.create((int)(blur.rows/factor), (int)(blur.cols/factor), CV_8UC1);
		
		sampleUchar(blur.data,img.data, factor, blur.cols,  blur.rows);
		preSigma2 = curSigma2;
...
//		cv::waitKey();
  ...
  unsigned int lineIDInScaleLineVec = 0;
	float dx, dy;
//	cout<<"edLineVec_[0]->lines_.numOfLines="<<edLineVec_[0]->lines_.numOfLines<<endl;
