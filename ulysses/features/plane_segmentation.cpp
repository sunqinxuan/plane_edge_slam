...
namespace ulysses
{
	double PlaneSegmentation::extractPlanes(Scan *scan)
	{
		...
		if(debug)
		{
			fp<<"regions: "<<regions.size()<<std::endl;
			fp<<"inlier_indices: "<<inlier_indices.size()<<std::endl;
			fp<<"boundary_indices: "<<boundary_indices.size()<<std::endl;
		}
...
//		{
//			if(label_indices[i].indices.size()<inliers) continue;
//			cout<<i<<endl;
...
//		cv::waitKey();
		...
		{
			if(inlier_indices[i].indices.size()<inliers) continue;
			...
			plane->ptr_normals=scan->normals();
			plane->ptr_pixels=scan->pixels();
//			fitPlaneModel(plane);
