		...
		Node_InterpTree* getMaxLeaf()
		{
			...
			for(int i=0;i<leaves.size();i++)
			{
				...
				while(ptr!=root)
				{
					if(!ptr->null) 
					{
						Eigen::Vector3d nc=ptr->feature_cur->getDirection();
						Eigen::Vector3d nr=ptr->feature_ref->getDirection();
						double dc=ptr->feature_cur->getDistance().norm();
						double dr=ptr->feature_ref->getDistance().norm();
						error+=(acos(nc.dot(nr))+fabs(dc-dr));
					}
					ptr=ptr->parent;
				}
				if(error<min)
...
//				std::cout<<i<<" --- "<<leaves[i]<<" --- "<<error<<std::endl;
			}
			return leaves[idx];
		}
...
//			interp_tree->setDebug(debug); dbg=debug;
			interp_tree->setDebug(false); dbg=false;
		}
