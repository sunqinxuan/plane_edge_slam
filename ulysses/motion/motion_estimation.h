...
//		void addEdgePoints(Scan *scan, const std::vector<EdgePoint*> &edge_points_ref);
...
//		int num_edge_,num_observ_edge_;
	...
	{
	public:
		PlaneError(const Eigen::Vector4d& pi_ref, const Eigen::Vector4d& pi_cur, const Eigen::Matrix4d& sqrt_information)
			 : observed_pi_ref_(pi_ref), observed_pi_cur_(pi_cur), sqrt_information_(sqrt_information) {}

		// camera = [q_w,q_x,q_y,q_z,t_x,t_y,t_z];
		// plane  = [n1,n2,n3,d];
		// residual_ptr = [delta_n1,delta_n2,delta_n3,delta_d];
		template <typename T>
		bool operator()(const T* const camera_ref, const T* const camera_cur, T* residual_ptr) const 
		{
			Eigen::Quaternion<T> q_cr(camera_cur[0],camera_cur[1],camera_cur[2],camera_cur[3]);
			q_cr.normalize();
			Eigen::Matrix<T,3,1> t_cr(camera_cur[4],camera_cur[5],camera_cur[6]);
...
//			Eigen::Map<const Eigen::Matrix<T,4,1> > pi(plane);
			...
			Eigen::Matrix<T,3,3> R_cr=q_cr.toRotationMatrix();
			Eigen::Matrix<T,4,4> T_cr; // transform for planes;
			T_cr.setIdentity();
			T_cr.template block<3,3>(0,0)=R_cr;
			T_cr.template block<1,3>(3,0)=-t_cr.transpose()*R_cr;
...
//			residual = observed_pi_.template cast<T>()-T_cg.template cast<T>()*pi.template cast<T>();
			residual = observed_pi_cur_.template cast<T>()-T_cr.template cast<T>()*observed_pi_ref_.template cast<T>();
...
//			fp.close();
		...
		static ceres::CostFunction* Create(const Eigen::Vector4d& pi_ref, const Eigen::Vector4d& pi_cur, const Eigen::Matrix4d& sqrt_information)
		{
			return new ceres::AutoDiffCostFunction<PlaneError,4,7,7>
					(new PlaneError(pi_ref,pi_cur,sqrt_information));
		}

		EIGEN_MAKE_ALIGNED_OPERATOR_NEW

	private:
		const Eigen::Vector4d observed_pi_ref_, observed_pi_cur_;
		const Eigen::Matrix4d sqrt_information_;
	};

//	class PlaneErrorCostFunction : public ceres::SizedCostFunction<7,7,4>
//	{
//	public:
//		PlaneErrorCostFunction(const Eigen::Vector4d& pi_ref, const Eigen::Vector4d& pi_cur, const Eigen::Matrix4d& sqrt_information)
//			: observed_pi_ref_(pi_ref), observed_pi_cur_(pi_cur), sqrt_information_(sqrt_information) {}
//
...
//	};
	...
	{
	public:
		EdgeError(const Eigen::Vector3d& pi_ref, const Eigen::Vector3d& pi_cur, const Eigen::Matrix3d& sqrt_information)
			 : observed_pi_ref_(pi_ref), observed_pi_cur_(pi_cur), sqrt_information_(sqrt_information) {}
...
//			residual = observed_pi_.template cast<T>()-T_cg.template cast<T>()*pi.template cast<T>();
			residual = observed_pi_cur_.template cast<T>()
				     - R_cr.template cast<T>()*observed_pi_ref_.template cast<T>()-t_cr.template cast<T>();
			residual.applyOnTheLeft(sqrt_information_.template cast<T>());
...
//			fp.close();
		...
		static ceres::CostFunction* Create(const Eigen::Vector3d& pi_ref, const Eigen::Vector3d& pi_cur, const Eigen::Matrix3d& sqrt_information)
		{
			return new ceres::AutoDiffCostFunction<EdgeError,3,7,7>
					(new EdgeError(pi_ref,pi_cur,sqrt_information));
		}

		EIGEN_MAKE_ALIGNED_OPERATOR_NEW

	private:
		const Eigen::Vector3d observed_pi_ref_, observed_pi_cur_;
		const Eigen::Matrix3d sqrt_information_;
	};

//	class EdgeErrorCostFunction : public ceres::SizedCostFunction<7,7,3>
//	{
//	public:
//		EdgeErrorCostFunction(const Eigen::Vector3d& pi_ref, const Eigen::Vector3d& pi_cur, const Eigen::Matrix3d& sqrt_information)
//			: observed_pi_ref_(pi_ref), observed_pi_cur_(pi_cur), sqrt_information_(sqrt_information) {}
//
//		virtual bool Evaluate(double const* const* parameters, double* residuals, double** jacobian) const;
//
...
//	};
	...
	public:
		...
		{ 
			...
			settings["MotionEstimation.usePlane"]>>use_plane_; 
			settings["MotionEstimation.useEdge"]>>use_edge_; 
			settings["MotionEstimation.feature_constant"]>>feature_constant; 
		    settings["MotionEstimation.max_iter_icp"]>>max_iter_icp;
		    settings["MotionEstimation.max_iter_lm"]>>max_iter_lm;
			settings.release();
...
//		bool alignOccludingLines(Scan *scan, Transform &Tcr);
		bool estimate(Scan *scan, boost::shared_ptr<pcl::visualization::PCLVisualizer> vis);
		// for debugging
...
//		int max_iterations;
		...
		std::vector<EdgePoint*> edge_points_ref;

		bool use_plane_,use_edge_;
...
//		Map *map;
		...
		double optimize(Scan *scan);
//		void buildProblem(PlaneLineBA* plane_line_ba, ceres::Problem* problem);
		void buildCorrespondence(Scan *scan_cur);

//		void buildProblem(Scan *scan, ceres::Problem* problem);
