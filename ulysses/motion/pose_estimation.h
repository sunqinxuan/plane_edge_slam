...
#include "types/map.h"
#include "motion/pose_estimation.h"
...
//		virtual void linearizeOplus();
	...
	public:
		...
		PoseEstimation(const std::string &settingFile)
		{
			remove("pose_estimation.txt");
			cv::FileStorage settings(settingFile.c_str(),cv::FileStorage::READ);
			settings["debug"]>>debug; 
			settings["PoseEstimation.max_iter_icp"]>>max_iter_icp;
			settings["PoseEstimation.max_iter_lm"]>>max_iter_lm;
			settings.release();
...
//		Transform alignPlanes(std::vector<PlanePair> matched_planes);
		...
		void estimate(Scan *scan, boost::shared_ptr<pcl::visualization::PCLVisualizer> vis);

//		void setMaxIterationICP(int i) {max_iter_icp=i;}
//		void setMaxIterationLM(int i) {max_iter_lm=i;}
...
//		Eigen::Vector3d getHSingulars() {return H_singularValues;}
	...
	private:
		...
		bool usePlane, useEdge;
//		bool useWeight;

		int max_iter_icp;
		int max_iter_lm;

		// corrsponding to scan_cur->edge_points;
...
//		void addCameraPose(g2o::SparseOptimizer *optimizer, Transform Tcr);
		...
		void buildCorrespondence(Scan *scan_cur, Transform Tcr);
